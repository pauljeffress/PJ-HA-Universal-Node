...
void mqttinbound() {
  if (!mqttClient.loop()) {      // check connection MQTT server and process MQTT subscription input
    ...
    digitalWriteHighPower(MQCON, LOW);     // switch off the MQTT/IP Connection LED, as we have lost connection.
    ...
    if(mqttCon){                      // Yes, we have re-established the MQTT/IP link so,
      digitalWriteHighPOwer(MQCON, HIGH_HP);   // turn on MQTT link indicator and
