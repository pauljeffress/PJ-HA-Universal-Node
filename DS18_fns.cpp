...
void DS18_check()
  ...
  #endif
      #ifdef DEBUG
      CONSOLE_PORT.print(" Requesting temperatures..."); // call sensors.requestTemperatures() to issue a global temperature request to all devices on the bus
      #endif
      sensors.requestTemperatures(); // Send the command to get temperatures
      #ifdef DEBUG
      CONSOLE_PORT.println("DONE");
      CONSOLE_PORT.print("Temperature for DS18 device 48 is: ");
      CONSOLE_PORT.println(sensors.getTempCByIndex(0)); // Why "byIndex"? You can have more than one IC on the same bus. 0 refers to the first IC on the wire
      CONSOLE_PORT.print("Temperature for DS18 Device 50 is: ");
      CONSOLE_PORT.println(sensors.getTempCByIndex(1)); // Why "byIndex"? 
      #endif
      delay(50);
  #ifndef SLEEPY
      }
...
void DS18_sleepyStuff()
  ...
  // with no red power LED, Arduino Pro Mini takes 20 uA @ Vin=4.0, 14 @ 3.3V
      CONSOLE_PORT.println(" going to sleep in 1 second");
      delay(1000);
